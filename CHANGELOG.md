...
- Swedish and Sony Taiga villages got a slight rework, few multi-workstation buildings are now separate (No more toolsmith and weaponsmith in the same building)
...
- several villages (Iberian, Swedish, Tudor, Rustic, Meadow and Classic) have received few slight changes in appearance, mostly for the sake of more uniform look of the whole structure.
- Mediterranean village got a bit of a refresh, with more consistent look.
- most village houses should no longer have connected roofs and be properly separated from each-other instead.
...
- one new exclusive structure has been added, that takes advantage of a special mechanic, only present in William Wythers' Expanded Ecosphere (previously called Overhauled Overworld, in 1.19). Beware the beasts!
...
- added a rare variant of the pillager galleon, with a Cthulhu face instead of the illager face embedded on the main sail as an homage to William Wythers, the original creator of the structure;
...
- remastered the tipi village (wooded badlands). The camp now includes actual loot, more detailed yurts, and in general more color;
